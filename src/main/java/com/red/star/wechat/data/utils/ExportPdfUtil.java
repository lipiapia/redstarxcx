...
import com.red.star.wechat.data.entity.vo.PDFBaseInfo;
import com.red.star.wechat.data.entity.vo.PDFColumn;
import com.red.star.wechat.data.entity.vo.PDFTable;
    ...
    public static byte[] export(String fileName, PDFBaseInfo pdfBaseInfo) throws Exception {
        ...
        if (!CheckUtil.isEmpty(pdfBaseInfo)) {
            ...
            if (!CheckUtil.isEmpty(pdfBaseInfo.getWatermark())) {
                //添加水印
                // addShuyinByWriter(writer, pdfBaseInfo.getWatermark());
                //通过PDF页面事件模式添加文字水印功能
//                writer.setPageEvent(new TextWaterMarkPdfPageEvent(pdfBaseInfo.getWatermark()));
...
//        waterMarkImage.setRotation(-10);// 旋转 弧度
